...
def parse_json_from_llm(response_str: str) -> dict:
    """
    Hàm tiện ích thông minh để trích xuất JSON từ chuỗi trả về của LLM.
    Nó có thể xử lý các trường hợp:
    1. JSON nằm trong khối markdown (```json ... ```).
    2. Có text rác trước hoặc sau JSON.
    3. Chuỗi là JSON thuần túy.
    """
    if not isinstance(response_str, str):
        return {}

    # Case 1: Tìm JSON trong khối markdown ```json ... ```
    match = re.search(r"```json\n(.*?)\n```", response_str, re.DOTALL)
    if match:
        json_str = match.group(1).strip()
    else:
        # Case 2 & 3: Lấy toàn bộ chuỗi nếu không có markdown
        json_str = response_str.strip()

    try:
        # Cố gắng parse chuỗi JSON đã tìm thấy
        parsed_json = json.loads(json_str)
        return parsed_json
    except json.JSONDecodeError:
        logger.warning(f"⚠️ [JSON Parser] Không thể parse JSON. Chuỗi đầu vào: {json_str[:200]}...")
        # Thử một lần nữa bằng cách tìm cặp ngoặc nhọn đầu tiên và cuối cùng
        try:
            start = json_str.find('{')
            end = json_str.rfind('}')
            if start != -1 and end != -1:
                cleaned_str = json_str[start:end+1]
                parsed_json = json.loads(cleaned_str)
                return parsed_json
        except json.JSONDecodeError:
            logger.error(f"❌ [JSON Parser] Thất bại hoàn toàn. Trả về dict rỗng.")
            return {} # Trả về dict rỗng để không làm crash flow
