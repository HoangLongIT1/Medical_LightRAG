...
# Lazy-loaded embedding models (initialized on first use)
_dense_model = None
_sparse_model = None
...
def _get_embedding_models():
    """
    Get embedding models, initializing them lazily on first call.
    Uses fastembed for dense, sparse, and late interaction models.
    """
    ...
    if _dense_model is None:
        try:
            ...
            logger.info("[Memory] Embedding models initialized")
        except ImportError:
            logger.error("[Memory] fastembed not installed. Memory features will not work.")
            raise
